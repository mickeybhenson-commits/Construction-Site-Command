...
def calculate_runoff_gal(precip_inches):
    # Rational Method: Q = C * I * A (C=0.45 for disturbed clay/silt)
    runoff_ft3 = (0.45 * (precip_inches/12) * (ACRES * 43560))
    return int(runoff_ft3 * 7.48)

# Historical Validation Data (Finalized 11:00 PM Yesterday)
hist_report = {
    "period": "Nov - Dec 2025",
    "total_rain": "4.12\"",
...
# COMMAND ADVISORY: VESSEL DEPLOYMENT MATH
wed_gal = calculate_runoff_gal(0.55)
st.error(f"**Vessel Advisory:** Wednesday's 0.55\" rain will generate ~{wed_gal:,} Gallons of runoff. Filtration vessel must be operational by 06:00 Wed.")import streamlit as st
import json
import pandas as pd
import datetime as dt
from pathlib import Path
